        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              role="combobox"
              aria-expanded={open}
              className="w-full justify-between border-2 pl-4 pr-4 py-7 text-left font-normal rounded-2xl
            ...
            >
              <div className="flex w-full items-center gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-100 dark:bg-violet-900/70 transition-colors group-hover:bg-violet-200 dark:group-hover:bg-violet-800 animate-shimmer">
                  <Search className="h-4 w-4 text-violet-600 dark:text-violet-300" />
                </div>
                <input
              ...
              <div className="flex gap-2 items-center">
                {query && (
                  <span 
                    className="flex h-7 w-7 items-center justify-center rounded-full bg-violet-100 dark:bg-violet-900/70 cursor-pointer hover:bg-violet-200 dark:hover:bg-violet-800 transition-colors hover:rotate-90 transition-transform duration-300"
                    onClick={clearSearch}
                  >
                ...
                <span 
                  className={`flex h-7 w-7 items-center justify-center rounded-full ${isListening ? 'bg-red-100 dark:bg-red-900/70 scale-110' : 'bg-violet-100 dark:bg-violet-900/70'} cursor-pointer hover:bg-violet-200 dark:hover:bg-violet-800 transition-all duration-300 hover:scale-110`}
                  onClick={toggleListening}
                >
                  <Mic className={`h-3.5 w-3.5 ${isListening ? 'text-red-600 dark:text-red-400 animate-pulse' : 'text-violet-600 dark:text-violet-300'}`} />
                  {isListening && (
          ...
          <PopoverContent 
            className="w-full p-0 rounded-xl border-violet-200 dark:border-violet-800/50 backdrop-blur-md bg-white/80 dark:bg-black/80 shadow-[0_8px_30px_rgba(142,85,255,0.12)] dark:shadow-[0_8px_30px_rgba(142,85,255,0.1)]" 
            align="start"
          >
